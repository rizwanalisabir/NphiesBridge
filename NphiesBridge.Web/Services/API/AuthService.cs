...
namespace NphiesBridge.Web.Services.API
{
    public class AuthService
    {
        ...
        public async Task<AuthResponseDto?> LoginAsync(LoginDto loginDto)
        {
            ...
            {
                Console.WriteLine($"Provider login error: {ex.Message}");
                return null;
            }
        }
        ...
        public void Logout()
        {
            // Synchronous version for controller use
            ClearToken();
            ClearUserInfo();
        }
        ...
        {
            return _httpContextAccessor.HttpContext?.Session.GetString("ProviderAuthToken");
        }

        public UserInfoDto? GetCurrentUser()
        {
            var userJson = _httpContextAccessor.HttpContext?.Session.GetString("ProviderCurrentUser");
        ...
        {
            _httpContextAccessor.HttpContext?.Session.SetString("ProviderAuthToken", token);
        }

        private void StoreUserInfo(UserInfoDto user)
        {
            var userJson = JsonSerializer.Serialize(user);
            _httpContextAccessor.HttpContext?.Session.SetString("ProviderCurrentUser", userJson);
        }

        private void ClearToken()
        {
            _httpContextAccessor.HttpContext?.Session.Remove("ProviderAuthToken");
        }

        private void ClearUserInfo()
        {
            _httpContextAccessor.HttpContext?.Session.Remove("ProviderCurrentUser");
        }
    }
}